...
function validUserName(username:String):any {
    if(!username) {
        return {
...
function validEmail(email:String):any {
...
function validPassword(password:String):any {
    if(!password) {
        return {
...
export default function(req:Request,res:Response,next:NextFunction) {
    const { user } = req.body;
    if(!user) return res.status(400).send({ error: "An Error Has Occured" });
